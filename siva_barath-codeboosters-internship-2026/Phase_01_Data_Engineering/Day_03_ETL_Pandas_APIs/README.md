...
---
ETL Best Practices
Always check shape before and after — `print(df.shape)` at start and end
Never modify raw data — `df = raw_df.copy()`
Log every step — `print()` what changed and how many rows affected
Validate after each fix — `isnull().sum()` should be 0 after fillna
Handle errors gracefully — `errors='coerce'` on type conversions
Document your choices — comment WHY you chose median vs 0 for fillna
---
Common Errors — Day 3
Error	Cause	Fix
...
---
Practice Questions
Explain ETL using a hospital example (patient records).
A DataFrame has 500 rows. After `df.dropna()` it has 412. What does this tell you?
Write code to remove rows where `customer_name` AND `product` are both the same.
What is the difference between `fillna(0)` and `fillna(df['col'].median())`?
Write code to call the weather API for 'Delhi' and print the temperature.
What does `response.status_code == 401` mean? How do you fix it?
---
